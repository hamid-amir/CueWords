...
def main(model_ckpt, dataset_path, do_ablation):
  OUTPUT_DIR = "results_cm/" if not do_ablation else "results_cm/ablation/"
  ...
  for num_cues in range(MIN_NUM_CUES, MAX_NUM_CUES+1):
    ...
    df = d2cm.retrieveCM(ablation_study=do_ablation)
...
if __name__ == '__main__':
  parser = argparse.ArgumentParser(description='Extracts context mixing scores given a model and a dataset')
