...
class GemmaAttention(nn.Module):
    ...
    def forward(
        ...
        value_states = repeat_kv(value_states, self.num_key_value_groups)  # shape: (bsz, num_head, q_len, d_head)

        # Start adding by Hamidreza
        value_states = value_states.permute(0, 2, 1, 3).reshape(bsz, q_len, self.num_heads * self.head_dim)  # shape: (B,T,d)
        if patch_value_vector is not None:
            value_states[:, patch_value_position[0]:patch_value_position[1], :] = patch_value_vector[:, patch_value_position[0]:patch_value_position[1], :]
        value_vectors = value_states  # shape: (B,T,d)
        value_states = value_states.reshape(bsz, q_len, self.num_heads, self.head_dim).permute(0, 2, 1, 3)  # shape: (bsz, num_head, q_len, d_head)
        ...
        if output_value_vectors:
            return attn_output, attn_weights, past_key_value, value_vectors
        else:
...
class GemmaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Self Attention
        hidden_states, self_attn_weights, present_key_value, value_vectors = self.self_attn(   # modified by Hamidreza
            hidden_states=hidden_states,
            attention_mask=attention_mask,
            position_ids=position_ids,
            past_key_value=past_key_value,
...
class GemmaModel(GemmaPreTrainedModel):
    ...
    def forward(
        ...
        for i, decoder_layer in enumerate(self.layers):   # Modified by Hamidreza
        ...
        if not return_dict:
            return tuple(v for v in [hidden_states, next_cache, all_hidden_states, all_self_attns, all_value_vectors] if v is not None)  # Modified by Hamidreza
